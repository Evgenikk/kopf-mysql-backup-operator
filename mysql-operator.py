...
def mysql_on_create(body, spec, **kwargs): # Функция, которая будет запускаться при создании объектов тип MySQL
    ...
    kopf.adopt(persistent_volume, owner=body)
    kopf.adopt(persistent_volume_claim, owner=body)
    ...
    # Создаем mysql PVC:
    api.create_namespaced_persistent_volume_claim('default', persistent_volume)
    # Создаем mysql SVC:
